...
### MODIFIED FOR TEST PURPOSES 2023 by Maryam Mehdizadeh - AEHRC CSIRO
from tensorflow.keras.optimizers import Adam
from tensorflow.keras.initializers import RandomNormal
from tensorflow.keras.models import Model
from tensorflow.keras.layers import Input
...
def summarize_performance(step, g_model, dataset, n_samples=3):
	...
	for i in range(n_samples):
		...
		save_img("./noisy_" + str(i) + "_" +  str(step+1)+ ".png", tf.concat([X_realAi, X_fakeBi, X_realBi],1))
...
def summarize_performance_test(step, g_model, dataset, n_samples=47):
	# select a sample of input images
...
def train(d_model, g_model, gan_model, dataset, n_epochs=100, n_batch=2):
...
start = time.time()
dataset = load_real_samples('./noisy_averaged_data.npz')
finish = time.time() - start
