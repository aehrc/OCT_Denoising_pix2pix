...
def summarize_performance_test(step, g_model, dataset, n_samples=200):
	...
	for i in range(n_samples):
		...
		X_realBi = img_to_array(X_realB[i])
		X_fakeBi = img_to_array(X_fakeB[i])
		save_img("./test_images_unet_patch_mae/test_" + str(i) + "_" +  str(step)+ ".png", tf.concat([X_realAi, X_fakeBi, X_realBi],1))
...
def train(d_model, g_model, gan_model, dataset, n_epochs=200, n_batch=4):
	...
	for i in range(70810,n_steps):
		...
		finish = time.time() - start
		print('>%d, d1[%.3f] d2[%.3f] g[%.3f] time[%d]' % (i+1, d_loss1, d_loss2, g_loss, finish))
...
g_model = load_model('./saved_models_patch_mae/model_073000.h5', compile=False)
# finish = time.time() - start
