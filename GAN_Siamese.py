...
#Codebase for SiameseGAN paper provided in: http://www.cs.utoronto.ca/~gkoch/files/msc-thesis.pdf
...
def siamese_model(input_shape):
	...
	prediction_layer = Lambda(lambda tensor: K.mean(tensor))
...
def define_gan(g_model, d_model, s_model, image_shape):
	...
	generated_images = g_model(left_inputs)
	discriminator_outputs = d_model(generated_images)
	siamese_outputs = s_model(inputs = [generated_images, right_inputs])
	model = Model(inputs=[left_inputs, right_inputs], outputs=[siamese_outputs,discriminator_outputs, generated_images])
	opt = Adam(lr=1E-4, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
	model.compile(loss=[wasserstein_loss, wasserstein_loss, perceptual_loss], optimizer=opt, loss_weights=[1, 5, 100])
	return model
...
# select a batch of random samples, returns images and target
def generate_real_zero_samples(dataset, n_samples, patch_shape):
	# unpack dataset
	trainA, trainB = dataset
	# choose random instances
	ix = randint(0, trainA.shape[0], n_samples)
...
def train(d_model, g_model, gan_model, dataset, n_epochs=200, n_batch=4):
	...
	for i in range(0,n_steps):
		...
		[_,_], y_real_zero = generate_real_zero_samples(dataset, n_batch, n_patch)
		...
		# d_loss_s = s_model.train_on_batch([X_fakeB, X_realB])
		g_loss, _, _, _ = gan_model.train_on_batch([X_realA, X_realB], [y_real_zero, y_real, X_realB])
...
d_model = define_discriminator(image_shape)
s_model = siamese_model(image_shape)
...
gan_model = define_gan(g_model, d_model, s_model, image_shape)
# train model
