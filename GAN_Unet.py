...
vgg = VGG16(include_top=False,   input_shape=(512,512,3), weights = '/Users/MEH011/OneDrive - CSIRO/Bracewell_Results/Pix2Pix_DnCNN/vgg16_weights_tf_dim_ordering_tf_kernels_notop.h5' )
...
def summarize_performance_test(step, g_model, dataset, n_samples=200):
	...
	for i in range(n_samples):
		...
		X_realBi = img_to_array(X_realB[i])
		X_fakeBi = img_to_array(X_fakeB[i])
		save_img("./test_images_unet/test_" + str(i) + "_" +  str(step)+ ".png", tf.concat([X_realAi, X_fakeBi, X_realBi],1))
...
g_model = load_model('./saved_models_unet/model_073000.h5', compile=False)
finish = time.time() - start

print("Loading model: " + str(finish) + " seconds")
# d_model = define_discriminator(image_shape)
