...
def summarize_performance_test(step, g_model, dataset, n_samples=200):
	...
	for i in range(n_samples):
		...
		X_realBi = img_to_array(X_realB[i])
		X_fakeBi = img_to_array(X_fakeB[i])
		save_img("./test_images_unet_patch_relu/test_" + str(i) + "_" +  str(step)+ ".png", tf.concat([X_realAi, X_fakeBi, X_realBi],1))
...
g_model = load_model('./saved_models_unet_patch_relu/model_073000.h5', compile=False)
